...
const subPenilaian = ref([]);
const bobotSubPenilaian = ref([]);
const namaSubPenilaian = ref([]);

const showSub = ref(false);

const sumSub = ref(0);

if (props.aspek.kriteriapenilaian.length > 0) {
    subPenilaian.value = Object.create(props.aspek.kriteriapenilaian)
    const AspekPenilaian = Object.create(props.aspek.kriteriapenilaian);
    AspekPenilaian.forEach((item, index) => {
        Form.sub_bobot_aspek.push(item.bobot)
        Form.sub_nama_aspek.push(item.nama)
    })
    showSub.value = true;
    sumSub.value = AspekPenilaian.length;
}
...
function deleteSlice(index){
    sumSub.value -= 1
    Form.sub_bobot_aspek.splice(index,1);
    Form.sub_nama_aspek.splice(index,1);
}
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2>Form Edit Aspek Penilaian</h2>
        </template>
                                    ...
                                    <ul class="list-item mt-2 space-y-3">
                                        <li v-for="(item, index) in sumSub" :key="item"
                                            ...
                                            <div class="flex flex-row gap-2">
                                                ...
                                                <PrimaryButton type='button' class="!text-xs !px-2 capitalize"
                                                    @click="deleteSlice(index)">X</PrimaryButton>
